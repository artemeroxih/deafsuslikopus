  ...
  return phone(os,
    '<div style="flex:1;display:flex;flex-direction:column;padding:0 24px">' +
      ...
      '<div class="ds-stack ds-g3" style="margin-top:24px">' +
        ...
        '<div style="display:flex;justify-content:flex-end"><button class="ds-btn ds-btn--sm ds-btn--ghost" style="padding:0" data-go="forgot">Забыли пароль?</button></div>' +
...
/** Шапка трёх экранов восстановления: логотип и стрелка назад. */
function recoverHead(title, sub) {
  return '<div style="padding-top:30px;display:flex;align-items:center;gap:12px">' +
      '<button class="ds-iconbtn ds-iconbtn--bare" aria-label="Назад">' + ic('arrowL', 20) + '</button>' +
      dsLogo(28, 18) + '</div>' +
    '<h1 class="ds-d2" style="font-size:1.55rem;margin-top:22px">' + title + '</h1>' +
    '<p class="ds-sm" style="margin-top:6px">' + sub + '</p>';
}

Mo.forgot = function (os) {
  return phone(os,
    '<div style="flex:1;display:flex;flex-direction:column;padding:0 24px">' +
      recoverHead('Восстановление пароля',
        'Введите почту, на которую заведён аккаунт. Пришлём ссылку для смены пароля.') +
      '<div class="ds-stack ds-g3" style="margin-top:24px">' +
        '<div class="ds-field"><label class="ds-label">Email</label>' +
          '<input class="ds-input" type="email" value="anna.k@example.ru" inputmode="email"></div>' +
        '<button class="ds-btn ds-btn--primary ds-btn--hero ds-btn--block">Отправить ссылку</button>' +
      '</div>' +
      // Отвечаем одинаково независимо от того, есть такая почта или нет:
      // иначе экран превращается в способ узнать, кто зарегистрирован.
      '<div class="ds-card" style="margin-top:18px;padding:14px">' +
        '<p class="ds-cap" style="line-height:1.5">Письмо придёт, только если аккаунт с такой почтой существует. ' +
        'Мы не сообщаем, заведён он или нет, — иначе по этому экрану можно было бы узнать чужую почту.</p></div>' +
      '<div class="ds-grow"></div>' +
      '<p class="ds-sm ds-center" style="padding-bottom:16px">Вспомнили пароль? <span style="color:var(--ds-accent);font-weight:600">Войти</span></p>' +
    '</div>');
};

Mo.forgotSent = function (os) {
  return phone(os,
    '<div style="flex:1;display:flex;flex-direction:column;padding:0 24px">' +
      recoverHead('Проверьте почту', 'Отправили ссылку на <b style="color:var(--ds-ink)">anna.k@example.ru</b>. Она действует час.') +
      '<div class="ds-center" style="margin-top:34px">' +
        '<span style="display:grid;place-items:center;width:72px;height:72px;border-radius:99px;' +
          'background:var(--ds-raised);color:var(--ds-accent);margin:0 auto">' + ic('send', 30) + '</span></div>' +
      '<div class="ds-stack ds-g3" style="margin-top:30px">' +
        '<button class="ds-btn ds-btn--secondary ds-btn--block">Открыть почту</button>' +
        // Кнопка выключена и честно говорит, сколько ждать: «отправить ещё раз»
        // без счётчика заставляет жать вслепую и слать письма пачками.
        '<button class="ds-btn ds-btn--ghost ds-btn--block" disabled style="opacity:.55">Отправить ещё раз через 0:47</button>' +
      '</div>' +
      '<div class="ds-card" style="margin-top:18px;padding:14px">' +
        '<div class="ds-row-f ds-g2" style="align-items:flex-start">' +
          '<span style="color:var(--ds-ink-3);display:flex;flex:none;margin-top:1px">' + ic('info', 16) + '</span>' +
          '<p class="ds-cap" style="line-height:1.5">Письма нет через пару минут — загляните в «Спам» и «Промоакции». ' +
          'Если и там пусто, напишите нам: ответим и восстановим доступ вручную.</p></div></div>' +
      '<div class="ds-grow"></div>' +
      '<p class="ds-sm ds-center" style="padding-bottom:16px">Ошиблись почтой? <span style="color:var(--ds-accent);font-weight:600">Ввести другую</span></p>' +
    '</div>');
};

Mo.reset = function (os) {
  return phone(os,
    '<div style="flex:1;display:flex;flex-direction:column;padding:0 24px">' +
      recoverHead('Новый пароль', 'Придумайте пароль, которым будете входить дальше.') +
      '<div class="ds-stack ds-g3" style="margin-top:24px">' +
        '<div class="ds-field"><label class="ds-label">Новый пароль</label>' +
          '<div style="position:relative;display:flex;align-items:center">' +
        ...
        '<div class="ds-field"><label class="ds-label">Повторите пароль</label>' +
          '<input class="ds-input" type="password" value="mypassword"></div>' +
        '<button class="ds-btn ds-btn--primary ds-btn--hero ds-btn--block" style="margin-top:4px">Сохранить и войти</button>' +
      '</div>' +
      // Смена пароля выкидывает чужие сессии — об этом надо предупредить
      // заранее, а не показывать список устройств, потерявших доступ.
      '<div class="ds-card" style="margin-top:18px;padding:14px;border-color:var(--ds-amber-edge)">' +
        '<div class="ds-row-f ds-g2" style="align-items:flex-start">' +
          '<span style="color:var(--ds-amber);display:flex;flex:none;margin-top:1px">' + ic('shield', 16) + '</span>' +
          '<p class="ds-cap" style="line-height:1.5">После смены пароля вход останется только на этом устройстве. ' +
          'На остальных — телефоне, планшете, компьютере — придётся войти заново.</p></div></div>' +
      '<div class="ds-grow"></div>' +
    '</div>');
};
